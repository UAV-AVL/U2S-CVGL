...
from multi_model.camp.sample4geo.hand_convnext.ConvNext.backbones.model_convnext_0 import convnext_tiny
...
class build_convnext(nn.Module):
    def __init__(self, num_classes, block=4, return_f=False, resnet=False, pos_scale=0.1, model_name='', backbone_model=None):
        super(build_convnext, self).__init__()
        self.return_f = return_f

        # if backbone_model==0:
        #     from multi_model.camp.sample4geo.hand_convnext.ConvNext.backbones.model_convnext_0 import convnext_tiny
        # elif backbone_model==1:
        #     from multi_model.camp.sample4geo.hand_convnext.ConvNext.backbones.model_convnext_0 import convnext_tiny
        ...
        if resnet:
            ...
            convnext_name = model_name
            print('using model_type: {} as a backbone'.format(convnext_name))
            if 'base' in convnext_name:
                self.in_planes = 1024
                self.convnext = create_model(convnext_name, pretrained=True,num_classes=0)
                ...
                self.in_planes = 2048
                self.convnext = create_model(convnext_name, pretrained=True,num_classes=0)
            elif 'pico' in convnext_name:
                self.in_planes = 512
                self.convnext = create_model(convnext_name,pretrained=True,num_classes=0)
            elif 'femto' in convnext_name:
                self.in_planes = 384
                self.convnext = create_model(convnext_name, pretrained=True, num_classes=0)
            elif 'nano' in convnext_name:
                self.in_planes = 640
                self.convnext = create_model(convnext_name, pretrained=True, num_classes=0)
            else:
                self.in_planes = 768
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(1, self.in_planes, 12, 12))
...
def make_convnext_model(num_class, block=4, return_f=False, resnet=False, pos_scale=0.1, model_name='', backbone_model=None):
    print('===========building convnext===========')
    model = build_convnext(num_class, block=block, return_f=return_f, resnet=resnet, pos_scale=pos_scale, model_name=model_name, backbone_model=backbone_model)
    return model
